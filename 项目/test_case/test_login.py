...
class TestLoin():
    ...
    def test_login_with_wrong_account(self, login):
        with allure.step("1、输入错误账号"):
            login.input_account(Base.get_data(login, file_name='test_login.yaml')['test_login']['incorrect_account'])
        with allure.step("2、输入错误密码"):
            login.input_password(Base.get_data(login, file_name='test_login.yaml')['test_login']['incorrect_password'])
        with allure.step("3、点击登录按钮"):
            login.click_login_btn()
            time.sleep(0.5)
        with allure.step("4、断言"):
